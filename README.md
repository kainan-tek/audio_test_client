...
- **C++ 标准**: C++17
- **构建系统**: Android.mk（推荐）/ Android.bp

### 平台版本兼容性

**Android.mk 方式（推荐）**：
- 自动检测 `PLATFORM_VERSION` (14/15/16)
- 自动适配不同 Android 版本的接口差异
...
- `audio_test_client.h` - 类声明和接口定义
- `audio_test_client.cpp` - 核心功能实现
- `Android.mk` - Make 构建配置（推荐）
...
2. 实现 `execute()` 纯虚函数
3. 在 `AudioOperationFactory` 中注册新模式
4. 更新命令行解析器和帮助信息
